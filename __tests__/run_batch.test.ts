...
describe("runBatch", () => {
  ...
  describe("context.batch", () => {
    it("should be undefined before runBatch", async () => {
      expect(firestoreSafe.context.batch).toBeUndefined();
    });

    it("should be assigned in runBatch", async () => {
      await firestoreSafe.runBatch(async (batch) => {
        expect(firestoreSafe.context.batch).toBe(batch);
      });
    });

    it("should be undefined after runBatch", async () => {
      await firestoreSafe.runBatch(async () => {
        expect(firestoreSafe.context.batch).not.toBeUndefined();
      });
    ...
    it("should be error transaction in runBatch", async () => {
      await firestoreSafe.runBatch(async () => {
        expect(
          firestoreSafe.runTransaction(async () => {
            dao.add({ title: "test" });
          })
        ).rejects.toThrow();
      });
  ...
  describe("Collection.context.batch", () => {
    ...
    beforeEach(async () => {
      batchAnotherDao = firestoreSafe.collection<TestDoc>({ path: anotherCollectionPath });
    });
