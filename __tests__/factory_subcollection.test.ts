import { FirestoreSafe, Encodable, Decodable, Collection } from "../src";
...
describe("Factory and Subcollection", () => {
  ...
  describe("FirestoreSafe.collectionFactory", () => {
    it("should has same encode function", async () => {
      const factory = firestoreSafe.collectionFactory<Book, BookDoc>({
        encode: encodeFunc
      });
  ...
  describe("FirestoreSafeCollectionFactory.create", () => {
    ...
    it("should has same context", async () => {
      expect(dao.context).toBe(firestoreSafe.context);
    });
