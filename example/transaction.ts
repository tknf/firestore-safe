...
class FriendRequestUsecase {
  ...
  constructor({
    ...
    this.firestoreSafe = firestoreSafe;
  ...
  async request(fromUserId: string, toUserId: string) {
    ...
    const friendDao = this.firestoreSafe.collection<UserFriend>({ path: `${ROOT_PATH}/user/${fromUserId}/friends` });
  ...
  async accept(friendRequest: FriendRequest) {
    ...
    const toUserFriendDao = this.firestoreSafe.collection<UserFriend>({
      path: `${ROOT_PATH}/user/${toUserFriend}/friends`
    });
    const fromUserFriendDao = this.firestoreSafe.collection<UserFriend>({
      path: `${ROOT_PATH}/user/${fromUserFriend}/friends`
    });
...
const main = async () => {
  ...
  const userDao = firestoreSafe.collection<User>({ path: `${ROOT_PATH}/user` });
  const friendRequestDao = firestoreSafe.collection<FriendRequest>({ path: `${ROOT_PATH}/request_friend` });
  ...
  const friendRequestUsecase = new FriendRequestUsecase({ firestoreSafe, userDao, friendRequestDao });
  // Start transaction
  // Bob send friend request to alice
  await firestoreSafe.runTransaction(async (_tx) => {
